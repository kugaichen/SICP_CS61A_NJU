...
CREATE TABLE siblings AS
  SELECT d1.name AS name1, d2.name AS name2, d1.height AS h1, d2.height AS h2 FROM dogs AS d1, dogs AS d2, parents AS p1, parents AS p2 WHERE d1.name = p1.child AND d2.name = p2.child AND p1.parent = p2.parent AND d1.name < d2.name ORDER BY d1.name DESC;
