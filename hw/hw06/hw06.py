...
class VendingMachine:
    ...
    def __init__(self,name,number):
        ...
        self.stock = 0 
        self.number = 0
        self.number_limit = number
        self.tryhands = 0
    
    def vend(self):
        if self.number ==0 and self.tryhands ==0:
            self.tryhands = self.tryhands + 1 
            return('Machine is out of stock.')
        elif self.number > self.number_limit:
            value = self.number - self.number_limit
            self.number = 0 
            return('Here is your {0} and ${1} change.'.format(self.name,value))
            
        elif self.number == self.number_limit:
            return('Here is your {0}.'.format(self.name))     
            
        elif self.number < self.number_limit:
            return('You must add ${0} more funds.'.format(self.number_limit - self.number))        
    ...
    def add_funds(self,num):
        if num < 15 :
            ...
            return('Current balance: ${0}'.format(self.number))
        else:
