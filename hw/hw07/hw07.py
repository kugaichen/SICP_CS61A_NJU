    ...
    def __str__(self):
        ...
        for i in range(0,len(self.list)):
          if i == 0:
            ...
            string = string + ' + ' + str(self.list[i]) + '*x^' + str(i)
          
        return string

    def __add__(self,other):
        length = max(len(self.list),len(other.list))
        new_list= [0] * length
        for i in range(len(self.list)):
          new_list[i] = new_list[i] + self.list[i]
        for i in range(len(other.list)):
          new_list[i] = new_list[i] + other.list[i]
          
        return Polynomial(new_list)
        
    def __mul__(self,other):
        new_len = len(self.list) + len(other.list)
        new_list = new_len * [0]
        for i in range(len(self.list)):
          for j in range(len(other.list)):
            new_list[i+j] = new_list[i+j] + self.list[i]*other.list[j]
...
def remove_duplicates(lnk):
    ...
    "*** YOUR CODE HERE ***"
    if lnk.rest:
      while lnk.rest.first == lnk.first:
...
def reverse(lnk):
    ...
    if lnk is not Link.empty:
      ...
      lnk_test = lnk
      while 1:
        list.append(lnk_test.first)
        if lnk_test.rest is not Link.empty:
          lnk_test = lnk_test.rest
        else:
          break
    
    def generate_lnk(lnk,list):
      lnk.first = list.pop()
      # print(lnk)
      if lnk.rest is not Link.empty:
        generate_lnk(lnk.rest,list)
...
class Tree:
    ...
    def __eq__(self,other): # Does this line need to be changed?
        ...
        """
        "*** YOUR CODE HERE ***"
        if self.label == other.label and len(self.branches) == len(other.branches):
...
def generate_paths(t, value):
    ...
    def create_path(t,value):
      if t is not None and  t.label != value:
        new_lst.append(t.label)
        # print('label: ',end='')
        # print(t.label)
        # print('new_lst: ',end='')
        # print(new_lst)
        if t is not t.is_leaf():
          for bra in t.branches:
            # new_lst.append(bra.label)
            create_path(bra,value)
            new_lst.pop()
      else:
        new_lst.append(t.label)
...
def balance_tree(t):
    ...
    def balance_one(t):
      ...
      for i in range(len(t.branches)):
        if t.branches[i].is_leaf() and t.branches[i].label > value:
          value = t.branches[i].label
    ...
    num1 = t.branches[0].label  
    num2 = t.branches[1].label
    
    if not t.branches[0].is_leaf():
      for bra in t.branches[0].branches:
        num1 = num1 + bra.label
        
    if not t.branches[1].is_leaf():
      for bra in t.branches[1].branches:
        num2 = num2 + bra.label
  
    if num1 > num2:
      t.branches[1].label = t.branches[1].label + num1 - num2
    if num2 > num1:
      t.branches[0].label = t.branches[0].label + num2 - num1
...
def install_camera(t):
    ...
    "*** YOUR CODE HERE ***"
    if not t.is_leaf():
      return 1 + install_camera(t.branches[0])
    else:
      return -1
