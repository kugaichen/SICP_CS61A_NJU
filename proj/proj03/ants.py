...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container is True and self.is_container is False:
                if place.ant.ant_contained is None:
                    place.ant.can_contain(self) 
                else:
                    assert place.ant is None, 'Two ants in {0}'.format(place)
            elif self.is_container is True and place.ant.is_container is False:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown != 0:
            ...
            pos = self.place
            object_bee = random_bee(pos.bees)
            if object_bee:
                self.chew_countdown = self.chew_duration
                object_bee.reduce_health(object_bee.health)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if self.is_scare > 1 and self.is_slow > 0:
                self.is_slow = self.is_slow -1 
                if gamestate.time % 2 == 0 :
                    self.is_scare = self.is_scare - 1
                    if self.place.entrance.is_hive == False:
                        self.move_to(self.place.entrance)
                        
            elif self.is_scare > 1 and self.is_slow == 0:
                self.is_scare = self.is_scare - 1
                if self.place.entrance.is_hive == False:
                        self.move_to(self.place.entrance)
                        
            elif self.is_slow > 0 and self.is_scare <= 1:
                if gamestate.time % 2 == 0:
    ...
    def slow(self, length):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        self.is_slow = self.is_slow + length
        # END Problem EC
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.is_scare == False:
            self.is_scare = length + 1          
        # END Problem EC
