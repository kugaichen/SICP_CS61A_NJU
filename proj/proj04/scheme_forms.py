...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        body = signature.rest 
        _lambda = do_lambda_form(Pair(body, expressions.rest), env)        
...
def make_let_frame(bindings, env):
    ...
    for x in bindings:
        validate_form(x,2,2)
        names.append(x.first)
        values.append(scheme_eval(x.rest.first, env))
    validate_formals(names)
    # print(bindings)
    # print(1)
    # names = scheme_eval(bindings.first,env)
    # values = scheme_eval(bindings.rest.first,env)
   
    # bindings.map(lambda item: validate_form(item, 2, 2))
    # names = bindings.map(lambda item: item.first)
    # values = bindings.map(lambda item: scheme_eval(item.rest.first, env))
    # validate_formals(names)
    # END PROBLEM 14
