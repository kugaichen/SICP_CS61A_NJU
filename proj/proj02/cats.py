...
def about(topic):
    ...
    def match_check(case,x):
        ...
        while 1:
            ...
            # print(x)
            # print(num_case)
            if lower(x[num_x]) != case[num_case]:
                num_x = num_x + 1 
                flag = 0
                num_case = 0
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0:
        ...
        while num <= len(typed_words) - 1 and num <= len(reference_words)-1:
            if typed_words[num] == reference_words[num]:
                index = index + 1
                num = num + 1  
            else:
                num = num + 1  
        result = index / len(typed_words)    
        if result == 1:
            return 100.0
        elif result == 0 :
            return 0.0
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for x in valid_words:
        distance = diff_function(x,user_word,limit)
        if distance <= record:
            record = distance
            index = x     
    if index:
...
def sphinx_swap(start, goal, limit):
    ...
    def check(num,length,start,goal):
        if num == length:
            return distance
        else:
            if start[num] != goal[num]:
                return 1 + check(num+1,length,start,goal)
            else:
                return check(num+1,length,start,goal)
            
    result = check(num,length,start,goal)
    return result
...
def feline_fixes(start, goal, limit):
    ...
    if not start and not goal: # Fill in the condition
        ...
        add_diff = feline_fixes(start,goal[1:],limit)  # Fill in these lines
        remove_diff = feline_fixes(start[1:],goal,limit)
...
def report_progress(typed, prompt, id, send):
    ...
    "*** YOUR CODE HERE ***"
    num = 0
    length = min(len(typed),len(prompt))
    length1 = len(typed)
    length2 = len(prompt)
    flag = 0
    while num < length1 and num < length2:
        score = accuracy(typed[num],prompt[num])
        if score == 100.0:
            flag = flag + 1 
            num = num + 1 
        else:
            break
    progress = flag / length2
...
def time_per_word(times_per_player, words):
    ...
    "*** YOUR CODE HERE ***"
    time_list =[]
    def time_count(times_per_player):
        for x in times_per_player:
            num = 0
            list =[]
            while num < len(x)-1:
                list = list + [x[num+1] - x[num-1]]
                num = num + 1 
            time_list.append(list)
            
        return time_list
    result = []   
    result.append(time_count(times_per_player))
    result.append(words)
    return result
...
def fastest_words(game):
    ...
    "*** YOUR CODE HERE ***"
    num = 0
    sequence_list = [[] for x in players]
    while num < len(all_times(game)[0]):
        flag = 20
        index = 0
        for x in all_times(game):
            if x[num] < flag:
                flag = x[num]
                index = all_times(game).index(x)
                
        sequence_list[index].append(all_words(game)[num])
        num = num + 1 
